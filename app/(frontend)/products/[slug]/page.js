...
const ProductPage = () => {
  ...
  useEffect(() => {
    ...
    const fetchProduct = async () => {
      try {
        const response = await fetch(`http://127.0.0.1:8000/products/get_product/${slug}`);
  ...
  useEffect(() => {
    const checkWishlist = async () => {
      ...
      try {
        const res = await fetch(`http://127.0.0.1:8000/wishlist/user/${user.id}`);
  ...
  const handleToggleWishlist = async () => {
    ...
    if (isWishlisted) {
      try {
        const res = await fetch(`http://127.0.0.1:8000/wishlist/wishlist/delete`, {
          method: "DELETE",
  ...
  useEffect(() => {
    const checkCart = async () => {
      ...
      try {
        const res = await fetch(`http://127.0.0.1:8000/cart/?user_id=${user.id}`);
