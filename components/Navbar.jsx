...
const Navbar = ({ Searchproducts }) => {
  const { showCart, setShowCart, cartItems, totalQty, setTotalQty, setCartItems, wishlistItems, setWishlistItems } = useStateContext();
  ...
  useEffect(() => {
    ...
    // Retrieve cart quantity from localStorage
              ...
              <div className="absolute right-0 mt-2 w-48 bg-white border rounded-md shadow-lg z-30">
                ...
                <button
                  ...
                  onClick={() => {
                    ...
                    localStorage.removeItem("cartQty");    // ✅ Clear cart quantity
                    localStorage.removeItem("wishlist");   // (Optional) Clear wishlist too if needed

                    setCartItems([]);     // ✅ Clear cart from context
                    setTotalQty(0);       // ✅ Reset cart quantity
